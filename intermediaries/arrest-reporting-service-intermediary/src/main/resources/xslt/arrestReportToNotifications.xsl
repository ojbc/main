...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                    ...
                    <b:Message>
                        <notification:NotificationMessage> 
                            ...
                            <nc:ActivityInvolvedPersonAssociation>
                                ...
                                <nc:PersonReference>
                                    <xsl:attribute name="s:ref">
                                    	<xsl:value-of select="generate-id(lexsdigest:Person)"/>
                                    </xsl:attribute>
                                </nc:PersonReference>
                                <xsl:choose>
                            ...
                            <xsl:if test="$lexsAssociations/nc:ResidenceAssociation or $lexsAssociations/lexsdigest:PersonContactAssociation">
                            	...
                            	<nc:ContactInformation>
                            		<xsl:attribute name="s:id">
                            			<xsl:value-of select="$contactInfoID"/>
                            		</xsl:attribute>
	...
	<xsl:template match="lexsdigest:TelephoneNumber">
		...
		<xsl:if test="nc:NANPTelephoneNumber">
			<nc:FullTelephoneNumber>
				<nc:TelephoneNumberFullID>(<xsl:value-of select="nc:NANPTelephoneNumber/nc:TelephoneAreaCodeID"/>)<xsl:value-of select="nc:NANPTelephoneNumber/nc:TelephoneExchangeID"/>-<xsl:value-of select="nc:NANPTelephoneNumber/nc:TelephoneLineID"/></nc:TelephoneNumberFullID>
			</nc:FullTelephoneNumber>
		</xsl:if>
    ...
    <xsl:template match="lexsdigest:ArrestOfficerAssociation" mode="arrestOfficial">
    	...
    	<j:ArrestOfficial>
    		<nc:RoleOfPersonReference>
    			<xsl:attribute name="s:ref">
    				<xsl:value-of select="generate-id($lexsDigest/lexsdigest:EntityPerson[lexsdigest:Person/@s:id=$personID]/j40:EnforcementOfficial)"/>
    			</xsl:attribute>
    		</nc:RoleOfPersonReference>
    	</j:ArrestOfficial>
    </xsl:template>
    <xsl:template match="lexsdigest:ArrestSubjectAssociation" mode="arrestSubject">
    	...
    	<j:ArrestSubject>
    		<nc:RoleOfPersonReference>
    			<xsl:attribute name="s:ref">
    				<xsl:value-of select="generate-id($lexsDigest/lexsdigest:EntityPerson/lexsdigest:Person[@s:id=$personID])"/>
    			</xsl:attribute>
    		</nc:RoleOfPersonReference>
    	</j:ArrestSubject>
    </xsl:template>
