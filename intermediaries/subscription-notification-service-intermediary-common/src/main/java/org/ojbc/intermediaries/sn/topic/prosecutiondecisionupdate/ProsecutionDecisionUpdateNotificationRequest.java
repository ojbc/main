...
public class ProsecutionDecisionUpdateNotificationRequest extends NotificationRequest {
	
	private static final Log log = LogFactory.getLog( ArrestNotificationRequest.class );
	
	
	public ProsecutionDecisionUpdateNotificationRequest(Message message) throws Exception{
	    this(message.getBody(Document.class));
	}

	public ProsecutionDecisionUpdateNotificationRequest(Document document) throws Exception {
	    super(document);
	...
	protected String getNotifyingSystemNameXPath() {
		// Add this to IEPD if required in notification message
		return "/b-2:Notify/b-2:NotificationMessage/b-2:Message/notfm-exch:NotificationMessage/notfm-ext:NotifyingProsecutionDecisionUpdate/notfm-ext:NotifyingActivityReportingSystemNameText";
	}
