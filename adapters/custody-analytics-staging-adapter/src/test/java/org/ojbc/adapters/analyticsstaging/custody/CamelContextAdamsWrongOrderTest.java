...
public class CamelContextAdamsWrongOrderTest {
	
	private static final Log log = LogFactory.getLog( CamelContextAdamsWrongOrderTest.class );
	...
	public void testCustodyStatusChangeReportService() throws Exception
	{
		...
		assertThat(person.getDomicileStatusTypeId(), is(1));
		...
		custodyStatusChanges = analyticalDatastoreDAO.getCustodyStatusChangesByBookingNumber("Booking Number");
		...
		assertThat(custodyStatusChange.getBookingId(), nullValue());
	...
	public void testBookingReportServiceRoute() throws Exception, IOException {
		...
		person = analyticalDatastoreDAO.getPerson(2);
		Assert.assertNotNull(person);
		...
		List<BehavioralHealthAssessment> behavioralHealthAssessments = analyticalDatastoreDAO.getBehavioralHealthAssessments(person.getPersonId());
