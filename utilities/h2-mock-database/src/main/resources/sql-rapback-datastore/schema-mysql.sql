...
CREATE TABLE TWO_FACTOR_USER (
                TWO_FACTOR_USER_ID INT AUTO_INCREMENT NOT NULL,
                USER_NAME VARCHAR(300),
                USE_GOOGLE_AUTH BOOLEAN DEFAULT false NOT NULL,
                DATE_CREATED TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
                LAST_UPDATED_DATE TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP NOT NULL,
                PRIMARY KEY (TWO_FACTOR_USER_ID)
);

CREATE UNIQUE INDEX two_factor_user_idx
 ON TWO_FACTOR_USER
 ( USER_NAME );
...
CREATE INDEX subscribingsystemidentifier_idx
 ON subscription
...
CREATE INDEX identifier_name_idx
 ON SUBSCRIPTION_SUBJECT_IDENTIFIER
 ( IDENTIFIERNAME );

CREATE INDEX identifier_value_idx
 ON SUBSCRIPTION_SUBJECT_IDENTIFIER
 ( IDENTIFIERVALUE );
...
CREATE TABLE SUBSCRIPTION_PROPERTIES (
                ...
                PROBATION_INDICATOR BOOLEAN DEFAULT FALSE NOT NULL,
                PAROLE_INDICATOR BOOLEAN DEFAULT FALSE NOT NULL,
                DUAL_SUPERVISION_INDICATOR BOOLEAN DEFAULT FALSE NOT NULL,
                PRIMARY KEY (ID)
);
...
ALTER TABLE NSOR_FIVE_YEAR_CHECK MODIFY COLUMN FIVE_YEAR_CHECK_FILE MEDIUMBLOB COMMENT 'The first initial results indicate whethe there is a match.';
...
ALTER TABLE NSOR_SEARCH_RESULT MODIFY COLUMN SEARCH_RESULT_FILE MEDIUMBLOB COMMENT 'The first initial results indicate whethe there is a match.';
...
ALTER TABLE NSOR_DEMOGRAPHICS MODIFY COLUMN DEMOGRAPHICS_FILE MEDIUMBLOB COMMENT 'The first initial results indicate whethe there is a match.';
...
ALTER TABLE `rapback_datastore`.`subscription_subject_identifier` 
ADD INDEX `combo_subID_name` (`SUBSCRIPTIONID` ASC, `IDENTIFIERNAME` ASC) VISIBLE;