...
<xsl:stylesheet version="2.0" 
    ...
    <xsl:template match="exchange:IncidentPersonSearchResults/ext:IncidentPersonSearchResult" >
        ...
        <tr class="clickableIncident"
            ...
            <xsl:variable name="incidentIdNoAgency">
                <xsl:call-template name="substring-before-last">
                    <xsl:with-param name="string1" select="ext:Incident/nc:ActivityIdentification/nc:IdentificationID" />
                    <xsl:with-param name="string2" select="'-'" />
                </xsl:call-template>
            </xsl:variable>

            <td><xsl:value-of select="$incidentIdNoAgency" /></td>
