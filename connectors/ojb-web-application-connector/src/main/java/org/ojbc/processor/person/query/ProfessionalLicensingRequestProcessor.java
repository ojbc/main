...
@Service
public class ProfessionalLicensingRequestProcessor extends RequestResponseProcessor implements CamelContextAware, DetailsQueryInterface {
	...
	private MessageProcessor professionalLicensingMessageProcessor;
	
	private OJBSamlMap OJBSamlMap;
	
	private boolean allowQueriesWithoutSAMLToken;
	
	private static final Log log = LogFactory.getLog( ProfessionalLicensingRequestProcessor.class );
	
	public String invokeRequest(DetailsRequest professionalLicensing, String federatedQueryID, Element samlToken) throws Exception
	{
		...
		//POJO to XML Request
		String professionalRequest = RequestMessageBuilderUtilities.createPersonQueryProfessionalRequest(professionalLicensing);
		
		//Create exchange
		Exchange senderExchange = new DefaultExchange(camelContext, ExchangePattern.InOnly);
		
		//Set exchange body to XML Request message
		senderExchange.getIn().setBody(professionalRequest);
		...
		professionalLicensingMessageProcessor.sendResponseMessage(camelContext, senderExchange);
	...
	public MessageProcessor getProfessionalLicensingMessageProcessor() {
		return professionalLicensingMessageProcessor;
	}

	public void setProfessionalLicensingMessageProcessor(
			MessageProcessor professionalLicensingMessageProcessor) {
		this.professionalLicensingMessageProcessor = professionalLicensingMessageProcessor;
	}
