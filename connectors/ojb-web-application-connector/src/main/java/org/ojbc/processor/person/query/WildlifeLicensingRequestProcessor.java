...
@Service
public class WildlifeLicensingRequestProcessor extends RequestResponseProcessor implements CamelContextAware, DetailsQueryInterface {
	...
	private MessageProcessor wildlifeLicensingMessageProcessor;
	
	private OJBSamlMap OJBSamlMap;
	
	private boolean allowQueriesWithoutSAMLToken;
	
	private static final Log log = LogFactory.getLog( WildlifeLicensingRequestProcessor.class );
	
	public String invokeRequest(DetailsRequest wildlifeLicensing, String federatedQueryID, Element samlToken) throws Exception
	{
		...
		//POJO to XML Request
		String wildlifeRequest = RequestMessageBuilderUtilities.createPersonQueryWildlifeRequest(wildlifeLicensing);
		
		//Create exchange
		Exchange senderExchange = new DefaultExchange(camelContext, ExchangePattern.InOnly);
		
		//Set exchange body to XML Request message
		senderExchange.getIn().setBody(wildlifeRequest);
		...
		wildlifeLicensingMessageProcessor.sendResponseMessage(camelContext, senderExchange);
	...
	public MessageProcessor getWildlifeLicensingMessageProcessor() {
		return wildlifeLicensingMessageProcessor;
	}

	public void setWildlifeLicensingMessageProcessor(
			MessageProcessor wildlifeLicensingMessageProcessor) {
		this.wildlifeLicensingMessageProcessor = wildlifeLicensingMessageProcessor;
	}
