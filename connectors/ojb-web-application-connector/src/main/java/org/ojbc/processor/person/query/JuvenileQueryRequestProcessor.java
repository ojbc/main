...
@Service
public class JuvenileQueryRequestProcessor extends RequestResponseProcessor implements CamelContextAware, DetailsQueryInterface {
	...
	private MessageProcessor juvenileQueryMessageProcessor;
	
	private OJBSamlMap OJBSamlMap;
	
	private boolean allowQueriesWithoutSAMLToken;
	
	private static final Log log = LogFactory.getLog( JuvenileQueryRequestProcessor.class );
	
	public String invokeRequest(DetailsRequest juvenileQueryRequest, String federatedQueryID, Element samlToken) throws Exception
	{
		...
		//POJO to XML Request
		Document juvenileQueryRequestPayload = RequestMessageBuilderUtilities.createJuvenileQueryRequest(juvenileQueryRequest.getIdentificationID(), juvenileQueryRequest.getIdentificationSourceText());
		
		//Create exchange
		Exchange senderExchange = new DefaultExchange(camelContext, ExchangePattern.InOnly);
		
		//Set exchange body to XML Request message
		senderExchange.getIn().setBody(juvenileQueryRequestPayload);
		...
		juvenileQueryMessageProcessor.sendResponseMessage(camelContext, senderExchange);
	...
	public MessageProcessor getJuvenileQueryMessageProcessor() {
		return juvenileQueryMessageProcessor;
	}

	public void setJuvenileQueryMessageProcessor(
			MessageProcessor juvenileQueryMessageProcessor) {
		this.juvenileQueryMessageProcessor = juvenileQueryMessageProcessor;
	}
