...
public class ExpungeDispositionRequestProcessor extends RequestResponseProcessor implements CamelContextAware{
	...
	public String invokeRequest(Disposition disposition, Element samlToken) throws Throwable {
		...
		//POJO to XML Request
		Document expungeRequestPayload = RequestMessageBuilderUtilities.createExpungeRequest(disposition);
		
		//Create exchange
		Exchange senderExchange = new DefaultExchange(camelContext, ExchangePattern.InOnly);
		
		//Set exchange body to XML Request message
		senderExchange.getIn().setBody(expungeRequestPayload);
