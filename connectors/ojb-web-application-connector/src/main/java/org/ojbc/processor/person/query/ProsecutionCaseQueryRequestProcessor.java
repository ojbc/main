...
@Service
public class ProsecutionCaseQueryRequestProcessor extends RequestResponseProcessor implements CamelContextAware, DetailsQueryInterface {
	...
	private MessageProcessor prosecutionCaseQueryRequestMessageProcessor;
	
	private OJBSamlMap OJBSamlMap;
	
	private boolean allowQueriesWithoutSAMLToken;
	
	private static final Log log = LogFactory.getLog( ProsecutionCaseQueryRequestProcessor.class );
	
	public String invokeRequest(DetailsRequest ProsecutionCaseQueryRequest, String federatedQueryID, Element samlToken) throws Exception
	{
		...
		log.debug("Prosecution Case ID from web portal: "  + ProsecutionCaseQueryRequest.getIdentificationID());
		log.debug("Prosecution Case Source Text from web portal: "  + ProsecutionCaseQueryRequest.getIdentificationSourceText());
		
		//POJO to XML Request
		String requestPayload = RequestMessageBuilderUtilities.createProsecutionCaseQueryRequest(
				ProsecutionCaseQueryRequest.getIdentificationID(), ProsecutionCaseQueryRequest.getIdentificationSourceText());
		...
		prosecutionCaseQueryRequestMessageProcessor.sendResponseMessage(camelContext, senderExchange);
	...
	public MessageProcessor getProsecutionCaseQueryRequestMessageProcessor() {
		return prosecutionCaseQueryRequestMessageProcessor;
	}


	public void setProsecutionCaseQueryRequestMessageProcessor(MessageProcessor prosecutionCaseQueryRequestMessageProcessor) {
		this.prosecutionCaseQueryRequestMessageProcessor = prosecutionCaseQueryRequestMessageProcessor;
	}
