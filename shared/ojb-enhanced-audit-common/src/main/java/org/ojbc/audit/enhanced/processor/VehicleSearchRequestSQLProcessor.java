...
public class VehicleSearchRequestSQLProcessor extends AbstractVehicleSearchRequestProcessor {

	private static final Log log = LogFactory.getLog(VehicleSearchRequestSQLProcessor.class);
	
	private EnhancedAuditDAO enhancedAuditDAO;
	
	private UserInfoSQLProcessor userInfoSQLProcessor;
	
	public void auditVehicleSearchRequest(Exchange exchange, @Body Document document)
	{
		try {
			...
			VehicleSearchRequest vehicleSearchRequest = processVehicleSearchRequest(document);
			
			vehicleSearchRequest.setUserInfofk(userInfoPk);
			
            String messageId = (String) exchange.getIn().getHeader("federatedQueryRequestGUID");
            vehicleSearchRequest.setMessageId(messageId);
            
			Integer vehicleSearchRequestPK = enhancedAuditDAO.saveVehicleSearchRequest(vehicleSearchRequest);
			
			for (String systemToSearch : vehicleSearchRequest.getSourceSystemsList())
			{
				Integer systemToSearchPK = enhancedAuditDAO.retrieveSystemToSearchIDFromURI(systemToSearch);
				
				if (vehicleSearchRequestPK != null && systemToSearchPK != null)
				{
					enhancedAuditDAO.saveVehicleSystemToSearch(vehicleSearchRequestPK, systemToSearchPK);
				}	
