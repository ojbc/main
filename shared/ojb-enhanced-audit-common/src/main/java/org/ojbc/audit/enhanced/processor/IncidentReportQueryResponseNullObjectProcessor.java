...
public class IncidentReportQueryResponseNullObjectProcessor extends AbstractIncidentReportQueryResponseProcessor{

	private static final Log log = LogFactory.getLog(IncidentReportQueryResponseNullObjectProcessor.class);
	...
	public void auditIncidentReportQueryResponseResponse(@Body Document document,  @Header(value = "federatedQueryRequestGUID")String messageID) {
		...
		try {
			...
			e.printStackTrace();
			log.error("Unable to audit incident report query response: " + ExceptionUtils.getStackTrace(e));
		}
