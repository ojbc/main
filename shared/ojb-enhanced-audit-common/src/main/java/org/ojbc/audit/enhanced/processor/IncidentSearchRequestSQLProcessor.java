...
public class IncidentSearchRequestSQLProcessor extends AbstractIncidentSearchRequestProcessor {

	private static final Log log = LogFactory.getLog(IncidentSearchRequestSQLProcessor.class);
	...
	public void auditIncidentSearchRequest(Exchange exchange, @Body Document document)
	{
		try {
			...
			incidentSearchRequest.setUserInfofk(userInfoPk);
			
            String messageId = (String) exchange.getIn().getHeader("federatedQueryRequestGUID");
            incidentSearchRequest.setMessageId(messageId);
            
			Integer incidentSearchRequestPK = enhancedAuditDAO.saveIncidentSearchRequest(incidentSearchRequest);
			
			for (String systemToSearch : incidentSearchRequest.getSystemsToSearch())
			{
				Integer systemToSearchPK = enhancedAuditDAO.retrieveSystemToSearchIDFromURI(systemToSearch);
				
				if (incidentSearchRequestPK != null && systemToSearchPK != null)
				{
					enhancedAuditDAO.saveIncidentSystemToSearch(incidentSearchRequestPK, systemToSearchPK);
				}	
