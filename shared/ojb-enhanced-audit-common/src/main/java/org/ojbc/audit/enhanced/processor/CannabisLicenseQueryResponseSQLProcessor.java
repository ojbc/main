...
public class CannabisLicenseQueryResponseSQLProcessor extends AbstractCannabisLicensingQueryResponseProcessor {

	private static final Log log = LogFactory.getLog(CannabisLicenseQueryResponseSQLProcessor.class);
	...
	public void auditCannabisLicenseQueryResponse(@Body Document document,  @Header(value = "federatedQueryRequestGUID")String messageID) {
		...
		try {
			...
			cannabisLicensingQueryResponse.setMessageId(messageID);
			
			Integer personQueryPk = enhancedAuditDAO.retrievePersonQueryIDfromMessageID(messageID);
			
			if (personQueryPk != null)
			{
				cannabisLicensingQueryResponse.setQueryRequestId(personQueryPk);
			}	
