...
public class SAMLAssertionBuilder {
    ...
    /**
     * This method will provide a SAML assertion as an OpenSAML assertion.  The assertion will be assigned if the 
     * keystore information is properly specified.
     * 
     * @param issuerString
     * @param inResponseTo
     * @param recipient
	 ...
	 */
	public Assertion createSamlAssertion(String issuerString, String inResponseTo, String recipient, String audienceRestriction, String authenticationMethod, String defaultCanonicalizationAlgorithm,
			String defaultRSASignatureAlgorithm, Map<SamlAttribute, String> customAttributes, boolean useUserGroupsAttribute) throws Exception
	{
		OpenSAMLBootstrap.bootstrap();
		...
		{	
			...
			if (!useUserGroupsAttribute) {
				...
				addAttributeUserGroupsToList(customAttributes, attributes);
			}
	...
	private void addAttributeUserGroupsToList(Map<SamlAttribute, String> customAttributes,
			List<AttributeBean> attributes) {
		List<SamlAttribute> groupAttributes = SamlAttribute.getGroupAttributes(); 
		
		Boolean containsCustomGroupAttributes = groupAttributes.stream()
				.anyMatch(samlAttribute -> StringUtils.isNotBlank(customAttributes.get(samlAttribute))); 
		
		if (containsCustomGroupAttributes) {
			AttributeBean attributeBean = new AttributeBean();
			attributeBean.setQualifiedName(SamlAttribute.Groups.getAttibuteName());
			
			List<Object> values = new ArrayList<Object>();
			for (SamlAttribute groupAttribute : groupAttributes) {
				
				if (StringUtils.isNotBlank(customAttributes.get(groupAttribute))) {
					if (groupAttribute != SamlAttribute.EmployerORI) {
						if (BooleanUtils.toBoolean(customAttributes.get(groupAttribute))) {
							values.add(groupAttribute.getGroupsAttributeValue());
						}
					}
					else {
						String agencyOri = customAttributes.get(groupAttribute); 
						String oriGroupValue = groupAttribute.getGroupsAttributeValue() + agencyOri; 
						values.add(oriGroupValue); 
					}
				}
			}
